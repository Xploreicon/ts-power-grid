...
function VerifyOtpContent() {
  ...
  const handleVerify = async () => {
    ...
    try {
      ...
      // Exchange the magic-link token for a Supabase session.
  ...
  const handleResend = async () => {
    ...
    if (!phone) { router.replace("/sign-in"); return; }
    setCanResend(false);
    setCountdown(60);
    setCode("");
    setError("");
    try {
      ...
      router.replace(`/verify-otp?token=${encodeURIComponent(data.otpToken)}&phone=${encodeURIComponent(phone)}`);
  ...
  return (
    <div className="space-y-8">
      <div className="space-y-2">
        <h1 className="text-3xl font-display font-bold text-navy-950">Enter your code</h1>
      ...
      <div className="text-center">
        {canResend ? (
          ...
          <p className="text-sm text-navy-400 font-sans">
            Resend in{" "}
            <span className="font-mono font-bold text-navy-700">{countdown}s</span>
          </p>
        )}
      </div>
    </div>
